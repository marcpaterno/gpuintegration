...
public:
  ...
  size_t get_num_active_regions(sycl::queue& q,
                                Region_characteristics<ndim>& regs);
...
template <size_t ndim>
size_t
Sub_regions_filter<ndim>::get_num_active_regions(
  sycl::queue& q,
  Region_characteristics<ndim>& regs)
{
...
template <size_t ndim>
Sub_regions_filter<ndim>::Sub_regions_filter(sycl::queue& q,
       ...
       if (activeRegions[tid[0]] == 1.) {
         ...
         for (size_t i = 0; i < ndim; ++i) {
           newActiveRegions[i * newNumRegions + interval_index] =
             dRegions[i * numRegions + tid[0]];
           newActiveRegionsLength[i * newNumRegions + interval_index] =
             dRegionsLength[i * numRegions + tid[0]];
         }
                                 ...
                                 Estimates& parent_ests)
{
  ...
  const size_t num_active_regions =
    get_num_active_regions(q, region_characteristics);

  if (num_active_regions == 0) {
    return 0;
  }

  double* filtered_leftCoord =
    sycl::malloc_device<double>(num_active_regions * ndim, q);
  double* filtered_length =
    sycl::malloc_device<double>(num_active_regions * ndim, q);
  int* filtered_sub_dividing_dim =
    sycl::malloc_device<int>(num_active_regions, q);

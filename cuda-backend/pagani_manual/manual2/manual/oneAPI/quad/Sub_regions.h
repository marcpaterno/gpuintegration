...
Sub_regions<ndim>::~Sub_regions(){
  ...
  template<size_t ndim>
  void Sub_regions<ndim>::host_device_init(sycl::queue& q, const size_t numRegions){
  ...
  template<size_t ndim>
  void Sub_regions<ndim>::print_bounds(){
   ...
   template<size_t ndim>
  double Sub_regions<ndim>::compute_region_volume(size_t const regionID){    
  double reg_vol = 1.;
  for(size_t dim = 0; dim < ndim; dim++){
  ...
  template<size_t ndim>
  double Sub_regions<ndim>::compute_total_volume(){
  double total_vol = 0.;
  ...
  quad::Volume<double, ndim>
   Sub_regions<ndim>::extract_region(size_t const regionID){
    ...
    for(size_t dim = 0; dim < ndim; dim++){
      ...
      regionID_bounds.highs[dim] = dLeftCoord[region_index] + dLength[region_index];
    }
    return regionID_bounds;  
  }*/

  template<size_t ndim>
  void Sub_regions<ndim>::uniform_split(sycl::queue& q, size_t _divs_per_d){
    ...
    //compiler does not allow me to capture *this in lambda below, error: static_assert type not device copyable
    double* LeftCoord = dLeftCoord;
    double* Length = dLength;
    
    size_t default_num_threads = 512;
    size_t numBlocks = (size_t)ceil((double)nregions / (double)default_num_threads);
    
    q.submit([&](auto &cgh) {
    
		
        sycl::stream str(262144, 4096, cgh);
        cgh.parallel_for(sycl::range<1>(nregions),
               [=](sycl::id<1> _reg){
		 size_t reg = _reg[0];

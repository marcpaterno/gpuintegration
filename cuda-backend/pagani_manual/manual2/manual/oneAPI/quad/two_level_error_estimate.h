...
template <typename T>
void
RefineError(sycl::queue&q, T* dRegionsIntegral,
              T* dRegionsError,
              T* dParentsIntegral,
              T* dParentsError,
              T* newErrs,
...
{
    q.submit([&](auto &cgh) {
        cgh.parallel_for(sycl::range<1>(currIterRegions),
               [=](sycl::id<1> gtid){    
...
void
two_level_errorest_and_relerr_classify(sycl::queue& q, Region_estimates<ndim>& current_iter_raw_estimates, 

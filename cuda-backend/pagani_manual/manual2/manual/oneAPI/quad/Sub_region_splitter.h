...
class Sub_region_splitter{
 ...
 Sub_region_splitter(sycl::queue& q, size_t size):num_regions(size){}
    ...
    void split(sycl::queue& q, Sub_regions<ndim>& sub_regions, const Region_characteristics<ndim>& classifiers);
    
    template <typename T>
    void divideIntervalsGPU(sycl::queue& q, T* genRegions, T* genRegionsLength, T* activeRegions, T* activeRegionsLength,
        int* activeRegionsBisectDim, size_t numActiveRegions, int numOfDivisionOnDimension);
};

template<size_t ndim>
void Sub_region_splitter<ndim>::split(sycl::queue& q, Sub_regions<ndim>& sub_regions, const Region_characteristics<ndim>& classifiers){

...
namespace quad {
    ...
    template<typename T>
    void
    set_device_array_range(sycl::queue& q, T* arr, const size_t& lindex, const size_t& rindex, const T& val){
      if(rindex < lindex){
          printf("r < right index %lu smaller than left index %lu index in quad::set_device_array_range:", rindex, lindex);
          exit(1);
      }
      
      q.submit([&](auto &cgh) {
          q.parallel_for(sycl::range<1>(rindex-lindex+1), [=] (sycl::id<1> i){
            arr[i+lindex] = val;
          });
      }).wait();
    }
    ...
    {
        IntegT* d_integrand = sycl::malloc_shared<IntegT>(d_integrand, sizeof(IntegT));
        memcpy(d_integrand, integrand, sizeof(IntegT));
        return d_integrand;
    }
    
    size_t total_device_mem(sycl::queue& q, size_t num_regions){
        auto device = q.get_device();
    ...
    dot_product(sycl::queue& q, T* list_a, T* list_b, size_t size){
        ...
        auto est_ev = oneapi::mkl::blas::column_major::dot(q, size, list_a, 1, list_b, 1 , res);
        est_ev.wait();
        double result = res[0];
        sycl::free(res, q);
        return result;
    ...
    reduction(sycl::queue& q, T* list, size_t size){
         ...
         sycl::free(res, q);
         return result;*/
         T res = dpl::experimental::reduce_async(dpl::execution::dpcpp_default, list, list + size).get();
         return res;
    }

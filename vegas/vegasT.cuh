...
bool PrecisionAchieved(double estimate, 
						double errorest, 
						double epsrel, 
						double epsabs){
...
template<typename IntegT, int ndim>
void vegas(IntegT integrand, double epsrel, double epsabs,/*double regn[]*/ int fcode,
           double ncall, double *tgral, double *sd,
           double *chi2a, int* status, int titer, int itmax, int skip, quad::Volume<double, ndim>* vol = nullptr)
{
...
// code works only  for (2 * ng - NDMX) >= 0)
	...
	for (it = itmax+1; it <= titer && (*status); it++) {
		...
		cudaMemset(result_dev, 0, 2 * sizeof(double));
		vegas_kernelF<IntegT, ndim> <<< nBlocks, nThreads>>>(d_integrand, ng, npg, xjac, dxg, result_dev, xnd,
		                                       xi_dev, d_dev, dx_dev, regn_dev, ncubes, it, sc,
		                                       sci,  ing, chunkSize, totalNumThreads,
		                                       LastChunk, fcode, time(NULL));
...
bool AdjustParams(double& ncall, int& totalIters){

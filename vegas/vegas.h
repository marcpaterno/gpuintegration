...
// dimensions It prints out a weighted integral estimate, variance, std.dev and
// chi2/dof after each dim of each iteration For this purpose we maintain three
// running sums over iterations (i) wsInt = running weighted sum of individual
// integral estimates (ii) wsIntSq = running weighted sum of squares of
// individual integral estimates (iii) Swgt = running sum of 1/Var[l] (also
// called weights)
...
double
newfuncgauss(/*double x[]*/ double x,
             double y,
             double l,
             double z,
             double m,
             ...
             double p)
{
  ...
  // for (j = 1; j <= ndim; j++)
  {
    tsum += (x - mu) * (x - mu) + (y - mu) * (y - mu) + (l - mu) * (l - mu) +
            (z - mu) * (z - mu) + (m - mu) * (m - mu) + (n - mu) * (n - mu) +
            (o - mu) * (o - mu) + (p - mu) * (p - mu);
  }
  tsum = tsum / (2 * sigma * sigma);
  tsum = exp(-tsum);
...
vegas(F integrand)
{
  ...
  int i, j, k, i1, iter, itcounter, npidx, idx, kk, npb;
  double rc, xn, xo;
  double realden2, newchi2, chi2 = 0.0;
  ...
  double IDim[ndim * ITMAX + 1], VDim[ndim * ITMAX + 1], witer[ITMAX + 1];
  double lbj, ubj, lb, ub, wgtp, fp, fbp, f2p, f2bp, uwidth,
    iterinteg[ITMAX + 1], itervariance[ITMAX + 1];
  double IIter, VIter;
  double Sw, wsInt, wsIntSq, wtIntEst;
  double d[N + 1][MXDIM + 1], dt[MXDIM + 1];
  double r[N + 1], x[MXDIM + 1] /*, y[MXDIM + 1]*/, xi[MXDIM + 1][N + 1],
    xin[N + 1];
  ...
  for (iter = 1; iter <= ITMAX; iter++) {
    ...
    for (i = 1; i <= ndim; i++) {
      ...
      for (j = 0; j < N; j++) {
        ...
        for (npidx = 1; npidx <= npb; npidx++) {
          wgtp =
            (ubj - lbj) *
            N; // wgtp will be w(x) when we finish picking x in each iteration
          temp = a * seed + c;
          seed = temp & (m - 1);
          trn = (double)seed / (double)m;
          // printf("inside trn:  %d  %d  %d   %e\n", iter, sliceId, npidx,
          // trn); x[i] = lbj + (rand() / (double)(RAND_MAX + 1.0)) * (ubj -
          // lbj);
          x[i] = lbj + trn * (ubj - lbj);
          ...
          // fp =  inv_realden2 * invndim * wgtp * REGIONVOLUME *
          // newfuncgauss(y); fp =  inv_realden2 * invndim * wgtp * REGIONVOLUME
          // * integrand(y);
          ...
          fp = inv_realden2 * invndim * wgtp * REGIONVOLUME *
               std::apply(integrand, actual_y_array);

          f2p = fp * fp;
          fbp += fp;
          f2bp += f2p;
          // if ((i==5) && (j==38)) {
          //     printf("f   %.12e   fp   %.12e   f2p  %.12e   fbp  %.12e f2bp
          //     %.12e\n", funcgauss(y), fp, f2p, fbp, f2bp);
          // }
        }
        // Here fbp = Sum w(x)f(x) = N*sum(f(x)/p(x))and f2bp = N^2 *
        // sum((f(x)/p(x))^2)
        d[j + 1][i] = f2bp;
        ...
        // Calculations below are for the variance estimate of the integral
        // estimate within the slice math gives = 1/((npb)^2(npb-1) *
        // N^2)[npb*f2bp - fbp^2]

        f2bp *= npb;
        f2bp = sqrt(f2bp);
        VSlice[itcounter][j + 1] =
          (f2bp - fbp) * (f2bp + fbp); // accomplishes the math,
    ...
    if (iter > ITGRIDADJ) {
      ...
      Sw += witer[iter]; // Sw = Sum of 1/VDim[l] 1<=l<=i; weighted variance is
                         // inv of this
      wsInt += witer[iter] *
               iterinteg[iter]; // wsInt = sum w[l]*IDim[l] 1<=l<=itcounter
      wtIntEst =
        wsInt / Sw; // weighted integral estimate up to current iteration
      wsIntSq += witer[iter] * iterinteg[iter] *
                 iterinteg[iter]; // wsIntSq = sum w[l]*IDim[l]^2 1<=l<=i
      ...
      for (i1 = ITGRIDADJ + 1; i1 <= iter; i1++) {
        newchi2 +=
          witer[i1] * (iterinteg[i1] - wtIntEst) * (iterinteg[i1] - wtIntEst);
      }
      newchi2 = newchi2 / (1.0 * itcounter - 0.99999);
      // printf ("%18.12f %18.12f %18.12f %12.4f \n", iterinteg[iter], wtIntEst,
      // sqrt(1.0 / Sw), chi2);
    }
    ...
    //     printf("Grid Updated\n");
    //     printf("-------------------------------------------------------------------------------------------\n");
    //     printf("it   Unwtd Integral     Wtd Integral          StdDev
    //     Chi2\n");
    //     printf("--------------------------------------------------------------------------------------------\n");
    // }
  }
  ...
  printf(
    "Method: CheckRZU, ndim = %i , nslices = %i, ITMAX = %i, ITGRIDADJ = %e\n",
    ndim,
    N,
    ITMAX,
    ITGRIDADJ);

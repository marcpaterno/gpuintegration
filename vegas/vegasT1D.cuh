                    ...
                    if constexpr(DEBUG_MCUBES){
                        if(randoms != nullptr){
                           // size_t cube_id_offset = m*chunkSize;
                            ...
                            size_t index = cube_id*nums_per_cube + nums_per_sample*(k-1) + j-1;
                            randoms[index] = ran00;
                        }
                    }
...
// code works only  for (2 * ng - NDMX) >= 0)
        ...
        for (it = 1; it <= itmax && (*status) == 1; (*iters)++, it++) {
            ...
            cudaMemcpy(xi, 
                        xi_dev, 
                        sizeof(double) * (mxdim_p1) * (ndmx_p1), cudaMemcpyDeviceToHost); 
            cudaCheckError();
            
            cudaMemcpy(d, 
                        d_dev,  
                        sizeof(double) * (ndmx_p1) * (mxdim_p1), cudaMemcpyDeviceToHost) ; 
            cudaCheckError();
        ...
        for (it = itmax+1; it <= titer && (*status) == 1; (*iters)++, it++) {
            ...
            MilliSeconds time_diff = std::chrono::high_resolution_clock::now() - t0;
            unsigned int seed = static_cast<unsigned int>(time_diff.count()) + static_cast<unsigned int>(it);   
            vegas_kernelF <IntegT, ndim, DEBUG_MCUBES, GeneratorType><<< nBlocks, nThreads>>>(d_integrand, 
                        ng, 
                        npg, 
                        xjac, 
                        dxg, 

...
__global__ void vegas_kernel(IntegT* d_integrand,
                            int ng, int npg, double xjac, double dxg,
                             double *result_dev, double xnd, double *xi,
                             double *d, double *dx, double *regn, double ncubes,
...
template<typename IntegT, int ndim>
__global__ void vegas_kernelF(IntegT* d_integrand, int ng, int npg, double xjac, double dxg,
                              double *result_dev, double xnd, double *xi,
                              double *d, double *dx, double *regn, double ncubes,
                              int iter, double sc, double sci, double ing,
                              int chunkSize, uint32_t totalNumThreads,
...
template<typename IntegT, int ndim>
void vegas1D(IntegT integrand, double regn[],
...
// code works only  for (2 * ng - NDMX) >= 0)
        ...
        for (it = 1; it <= itmax; it++) {
                ...
                mcubes1D::vegas_kernel<IntegT, ndim> <<<nBlocks, nThreads>>>
                    (d_integrand, ng, npg, xjac, dxg, result_dev, xnd,
                                                      xi_dev, d_dev, dx_dev, regn_dev, ncubes,
                                                      it, sc, sci,  ing, chunkSize,
                                                      totalNumThreads, LastChunk);
  ...
  for (it = itmax+1; it <= titer; it++) {
                ...
                vegas_kernelF <IntegT, ndim><<< nBlocks, nThreads>>>(d_integrand, ng, npg, xjac, dxg, result_dev, xnd, xi_dev, d_dev, dx_dev, regn_dev, ncubes, it, sc,
                    ...
                    int skip)
{
        ...
        vegas1D<IntegT, ndim>(integrand, regn, ncall, &avgi, &sd, &chi2a, titer, itmax, skip);
          
        bool success = res.estimate/res.errorest <= epsrel;
        res.estimate = avgi;
        res.errorest = sd;
        res.chi_sq = chi2a;
        res.status = static_cast<int>(success);
        return {avgi, sd, 0};

...
__global__ 
void
kernel(int chunkSize, uint32_t totalNumThreads, int LastChunk, double* result_dev, double *block_reductions){
    uint32_t m = blockIdx.x * blockDim.x + threadIdx.x;
    uint32_t tx = threadIdx.x;
    double fbg = 0.;    //each block reduction yields 128*1.1 even though the second block shouldn'tbe getting it from all
    ...
    if(tx == 0){
        ...
        atomicAdd(&result_dev[0], fbg); //i thought this wouldn't be problematic, threads where m > totalNumThreads should have zeroes all over
    }
    
}

void Reduction(double ncall, int chunkSize, int ndim){
    Kernel_Params kernel_params(ncall, chunkSize, ndim);
    double* result = cuda_malloc_managed<double>(1);
    double* block_reductions = cuda_malloc_managed<double>(static_cast<size_t>(kernel_params.nBlocks));
    double fgb = 1.1;
    
    kernel<<<kernel_params.nBlocks, kernel_params.nThreads>>>
        (chunkSize, kernel_params.totalNumThreads, kernel_params.LastChunk, result, block_reductions);

...
TEST_CASE("Exclusive scan of array of size 8")
{
	...
	quad::cuda_memcpy_to_host<int>(custom_results.data(), out, size);
    ...
    quad::cuda_memcpy_to_host<int>(thrust_results.data(), out, size);
...
TEST_CASE("Exclusive scan of array of non-power-two size")
{
	...
	quad::cuda_memcpy_to_host<int>(h_out_thrust.data(), out_thrust, size);
    quad::cuda_memcpy_to_host<int>(h_out_custom.data(), out_custom, size);
...
TEST_CASE("Exclusvie scan of array of size 8 double type")
{
	...
	quad::cuda_memcpy_to_device<double>(d_arr, arr.data(), size);
	
	sum_scan_blelloch(out, d_arr, size);
	quad::cuda_memcpy_to_host<double>(custom_results.data(), out, size);
    ...
    quad::cuda_memcpy_to_host<double>(thrust_results.data(), out, size);

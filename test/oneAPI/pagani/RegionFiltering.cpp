...
bool regions_same(double* listA, double* listB, size_t indexA, size_t indexB, int ndim, size_t nregionsA, size_t nregionsB){
	
	for(int dim = 0; dim < ndim;  ++dim){
		bool diff = listA[indexA + dim * nregionsA] != Approx(listB[indexB + dim * nregionsB]);
		if(diff)
			return false;
	}
	return true;
...
TEST_CASE("get_num_active_regions: 100-region-list")
{
	...
	Sub_regions_filter<ndim> filter(num_regions);
	...
	SECTION("All active regions")
	{
		...
		quad::cuda_memcpy_to_device<double>(regions.active_regions, host_active_regions, num_regions);
...
{
	constexpr int ndim = 2;
	Sub_regions<ndim> coordinates(10);
	size_t n = coordinates.size;
	Region_characteristics<ndim> classifications(n);
	Sub_regions_filter<ndim> filter(n);
	Region_estimates<ndim> estimates(n);
	Region_estimates<ndim> parents(n/2);
	...
	quad::cuda_memcpy_to_host<double>(original_LeftCoord, coordinates.dLeftCoord, n*ndim);
	quad::cuda_memcpy_to_host<double>(original_Length, coordinates.dLength, n*ndim);
	
	quad::cuda_memcpy_to_device<double>(estimates.integral_estimates, integrals_mirror, n);
	quad::cuda_memcpy_to_device<double>(estimates.error_estimates, error_mirror, n);
	quad::cuda_memcpy_to_device<double>(parents.integral_estimates, parent_integrals_mirror, n/2);
	quad::cuda_memcpy_to_device<double>(parents.error_estimates, parent_error_mirror, n/2);
	quad::cuda_memcpy_to_device<int>(classifications.sub_dividing_dim, sub_dividing_dim, n);
	quad::cuda_memcpy_to_device<double>(classifications.active_regions, host_active_regions, n);
	...
	quad::cuda_memcpy_to_host<double>(filtered_integrals, parents.integral_estimates, num_active);
	quad::cuda_memcpy_to_host<double>(filtered_errors, parents.error_estimates, num_active);
	quad::cuda_memcpy_to_host<double>(LeftCoord, coordinates.dLeftCoord, num_active*ndim);
	quad::cuda_memcpy_to_host<double>(Length, coordinates.dLength, num_active*ndim);
	quad::cuda_memcpy_to_host<int>(new_subdiv_dim, classifications.sub_dividing_dim, num_active);
	...
	for(int i=0; i < n; ++i)
		CHECK(regions_same(LeftCoord, original_LeftCoord, i, i, ndim, num_active, n));
...
TEST_CASE("filter: region 15 is inactive")
{
	...
	CHECK(regions_same(LeftCoord, original_LeftCoord, 14, 14, ndim, num_active, n));
	CHECK(!regions_same(LeftCoord, original_LeftCoord, 15, 15, ndim, num_active, n));
	CHECK(regions_same(LeftCoord, original_LeftCoord, 15, 16, ndim, num_active, n));
}
...
TEST_CASE("filter: first and last regions are inactive")
{
	...
	CHECK(!regions_same(LeftCoord, original_LeftCoord, num_active-1, n-1, ndim, num_active, n));
}

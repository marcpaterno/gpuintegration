...
TEST_CASE("All finished regions")
{
  ...
  size_t num_active =
    filter.get_num_active_regions(regs.active_regions, num_regions);
  CHECK(num_active == 0);
}
...
TEST_CASE("Some sub-regions are finished")
{
  ...
  std::array<std::pair<size_t, size_t>, num_ranges_with_all_finished>
    finished_ranges = {{{0, 50}, {63, 71}, {101, 121}, {124, 125}, {127, 129}}};

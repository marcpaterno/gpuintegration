...
bool regions_same(double* listA, double* listB, size_t indexA, size_t indexB, int ndim, size_t nregionsA, size_t nregionsB){
	...
	for(int dim = 0; dim < ndim;  ++dim){
		...
		//printf("dim:%i %f, %f: bool:%i\n", dim, listA[index + dim * nregions], listB[index + dim * nregions], diff);
		if(diff)
			return false;
	}
	return true;
...
TEST_CASE("filter: all regions are active")
{
	...
	for(int i=0; i < n; ++i)
		CHECK(regions_same(LeftCoord.data(), original_LeftCoord.data(), i, i, ndim, num_active, n));
...
TEST_CASE("filter: region 15 is inactive")
{
	...
	CHECK(regions_same(LeftCoord.data(), original_LeftCoord.data(), 0, 0, ndim, num_active, n));
	CHECK(regions_same(LeftCoord.data(), original_LeftCoord.data(), 1, 1, ndim, num_active, n));
	CHECK(regions_same(LeftCoord.data(), original_LeftCoord.data(), 14, 14, ndim, num_active, n));
	CHECK(!regions_same(LeftCoord.data(), original_LeftCoord.data(), 15, 15, ndim, num_active, n));
	CHECK(regions_same(LeftCoord.data(), original_LeftCoord.data(), 15, 16, ndim, num_active, n));
}
...
TEST_CASE("filter: first and last regions are inactive")
{
	...
	//check index the index inactive region to see if filtered out, check surrounding indices to detect any errors 
	//for(int i =0; i < n; ++i)
	//	printf("region %i %f +- %f\n", i, filtered_integrals[i], filtered_errors[i]);
	...
	CHECK(!regions_same(LeftCoord.data(), original_LeftCoord.data(), 0, 0, ndim, num_active, n));
	CHECK(regions_same(LeftCoord.data(), original_LeftCoord.data(), 0, 1, ndim, num_active, n));
	CHECK(!regions_same(LeftCoord.data(), original_LeftCoord.data(), num_active-1, n-1, ndim, num_active, n));
}

...
TEST_CASE("Compute finished estimates")
{
	...
	auto integral_estimates = Kokkos::create_mirror_view(estimates.integral_estimates);
	auto error_estimates = Kokkos::create_mirror_view(estimates.error_estimates);
	auto active_regions = Kokkos::create_mirror_view(characteristics.active_regions);
    ...
    SECTION("All finished regions")
    {
        ...
        integration_result test = compute_finished_estimates(estimates, characteristics, true_iter_estimate);

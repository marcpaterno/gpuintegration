...
TEST_CASE("Data can be set on the device and accessed on host")
{
  ...
  {
    // passing by value to kernel invokes copy-constructor, which does deep, not
    // shallow copy thus values don't update when accessing the array on host
    CHECK(array[1] != 11);
    CHECK(array[3] != 33);
    CHECK(array[4] != 44);
  }

  set_vals_at_indices<int_array, int>
    <<<1, 1>>>(array.data(), d_indices, d_vals);
  cudaDeviceSynchronize();

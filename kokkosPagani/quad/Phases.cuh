...
/*void
RefineError(size_t numBlocks, size_t numThreads){
    //this is computed in kernel.cuh
    Kokkos::parallel_for( "Phase1", team_policy(numBlocks, numThreads), KOKKOS_LAMBDA (const member_type team_member) {
            
            int threadIdx = team_member.team_rank();
            int blockIdx = team_member.league_rank();        
            //printf("RefineError (multiple threads/regionBlock) block:%i thread:%i\n", blockIdx, threadIdx);
    });
}*/
    
/*void
Filter(size_t numBlocks, size_t numThreads){
    Kokkos::parallel_for( "Phase1", team_policy(numBlocks, numThreads), KOKKOS_LAMBDA (const member_type team_member) {
            
        int threadIdx = team_member.team_rank();
        int blockIdx = team_member.league_rank();        
        //printf("Filter (multiple threads/regionBlock) block:%i thread:%i\n", blockIdx, threadIdx);
    });
}*/

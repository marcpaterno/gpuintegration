...
GetGPUMemNeededForNextIteration(size_t numRegions, int NDIM){      
    ...
    size_t Doubles_Size = 8*(newActiveRegions + scanned + newActiveRegionsLength + parentExpansionEstimate + 
		parentExpansionErrorest + genRegions + genRegionsLength + regions + regionsLength + 
		regionsIntegral + regionsError + parentsIntegral + parentsError);
    //the above doesn't consider the intermediate memory needed
     return Ints_Size + Doubles_Size;
}    	
...
ComputeMax(ViewVectorDouble list){
    double max;
    Kokkos::parallel_reduce(list.extent(0), KOKKOS_LAMBDA(const int& index, double& lmax) {
                if(lmax < list(index))
                   lmax = list(index);
...
ComputeMin(ViewVectorDouble list){
    double min;
    Kokkos::parallel_reduce(list.extent(0), KOKKOS_LAMBDA(const int& index, double& lmin) {
                if(lmin > list(index))
                   lmin = list(index);

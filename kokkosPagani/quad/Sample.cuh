...
template <typename IntegT, typename T, int NDIM>
__device__ void
dummycomputePermutation(IntegT d_integrand,
                   int pIndex,
                   Bounds* b,
                   double* g,
                   gpu::cudaArray<T, NDIM>& x,
    ...
    double fun = gpu::apply(d_integrand(0), x)* (attributes(1));
    sdata(threadIdx) = fun; // target for reduction
...
template<typename IntegT, int NDIM>
__device__ void
    dummySample(IntegT d_integrand, 
		   int sIndex, 
		   Structures<double> constMem, 
		   int FEVAL, 
		   int NSETS, 
    ...
    if (pIndex < FEVAL) {
     dummycomputePermutation<IntegT, double, NDIM>(d_integrand, pIndex, region->bounds, g, x, sum, constMem, range, jacobian, generators, sdata, sBound, FEVAL, attributes, team_member);
    }
    ...
    if (threadIdx == 0) {
	  ...
	  //attributes(0) = vol
      r->avg = (attributes(0)) * sum[0];
      r->err = (attributes(0)) * ((errcoeff[0] * sum[1] <= sum[2] &&
                       errcoeff[0] * sum[2] <= sum[3]) ?
                        errcoeff[1] * sum[1] :
                        errcoeff[2] * max(max(sum[1], sum[2]), sum[3]));
    }*/

...
struct IntegrandAcrossIters{
    ...
    void UpdateValues(double newIntegral, double newError){
...
class Kernel{
  public:
		...
		Kokkos::parallel_for("DivideIntervalsGPU", team_policy(numBlocks, numThreads), KOKKOS_LAMBDA (const member_type team_member) {
			...
			if (tid < numActiveRegions) {
			  ...
			  for (int i = 0; i < numOfDivisionOnDimension; ++i) {
				...
				double interval_length = activeRegionsLength(bisectdim * numActiveRegions + tid) /  numOfDivisionOnDimension;
	...
	void ReturnLastIndexValues(ViewVectorDouble listA, ViewVectorDouble listB, double& lastA, double& lastB){
    ...
    GenerateActiveIntervals(ViewVectorDouble& dRegions,
		...
		ReturnLastIndexValues(activeRegions, scannedArray, lastElement, lastScanned);
		...
		if(numActiveRegions > 0){
			...
			ViewVectorDouble newActiveRegions("newActiveRegions",  numActiveRegions*NDIM);
			ViewVectorDouble newActiveRegionsLength("newActiveRegionsLength",  numActiveRegions*NDIM);
			ViewVectorInt newActiveRegionsBisectDim("newActiveRegionsBisectDim", numActiveRegions* numOfDivisionOnDimension);
    ...
    void FixErrorBudgetOverflow(ViewVectorDouble activeRegions, 
                                double& integral, 
                                double& error, 
    ...
    bool CheckZeroNumRegionsTermination(double& integral, double& error, double leaves_estimate, double leaves_errorest){
         if(numRegions == 0){
            integral = leaves_estimate;
            error = leaves_errorest;
			return true;
    ...
    bool 
	AdjustErrorThreshold(double ErrorestMarkedFinished, double MaxErrorestToAllowedToFinish, double percentRegionsToKeep, int& currentDirection, double& lastThreshold, double& threshold, double& minThreshold, double& maxThreshold, int& numDirectionChanges){
               ...
               int iteration)
    {
        ...
        estimateHasConverged =
            ...
            (iteration >= 2 ? sigDigitsSame(integrandAcrossIters.lastIterIntegral, 
                                        integrandAcrossIters.secondToLastIterIntegral, 
                                        leaves_estimate, 
                                        requiredDigits) : false) : true;
            //printf("it:%i estimateHasConverged:%i\n", iteration, estimateHasConverged);                            
            integrandAcrossIters.secondToLastIterIntegral = integrandAcrossIters.lastIterIntegral;
            integrandAcrossIters.lastIterIntegral = leaves_estimate;                            
            //printf("GetGPUMemNeededForNextIteration:%zu Available Mem:%zu percentage:%f\n", GetGPUMemNeededForNextIteration(numRegions, NDIM), GetAmountFreeMem(), 
			//	(double)((double)GetGPUMemNeededForNextIteration(numRegions, NDIM)/((double)GetAmountFreeMem())));
            //if(!estimateHasConverged || (GetGPUMemNeededForNextIteration(numRegions, NDIM) < GetAmountFreeMem())) 
			//	return;
            ...
            ViewVectorDouble unpolishedRegions("numRegions", numRegions);//int* unpolishedRegions = 0;
            //ViewVectorDouble scannedArray;//int* scannedArray  = 0; // de-allocated at the end of this function
            ...
            do{ 
                ...
                numActiveRegions = 0; 
                Filter(dRegionsError, unpolishedRegions, activeRegions, numRegions, ErrThreshold);
                numActiveRegions = ComputeNumUnPolishedRegions(unpolishedRegions);

                if(numActiveRegions <= targetRegionNum){               
                    iter_polished_estimate = iterEstimate - iter_finished_estimate -  KokkosBlas::dot(unpolishedRegions, dRegionsIntegral);
                
                    iter_polished_errorest = iterErrorest - iter_finished_errorest - KokkosBlas::dot(unpolishedRegions, dRegionsError);
    ...
    RelErrClassify(int heuristicID,
       ...
       // int heuristicID = positiveSemiDefinite;
        ...
        Kokkos::parallel_for("RefineError", numRegions, KOKKOS_LAMBDA (const int64_t index){
            ...
            size_t siblingIndex = index  + (inLeftSide*currIterRegions/2) - (inRightSide*currIterRegions/2);
            ...
            int PassRatioTest = heuristicID != 1 && selfErr < MaxErr(selfRes, epsrel, 1e-200);
    ...
    void GenerateInitialRegions(ViewVectorDouble& dRegions, ViewVectorDouble& dRegionsLength){
        ...
        //need c++17 for KOKKOS_LAMBDA that is defined as [=,*this] __host__ __device__, interestingly adding __host__ below causes problems
        Kokkos::parallel_for( "GenerateInitialRegions", team_policy(numBlocks, numThreads).set_scratch_size(0, Kokkos::PerTeam(shMemBytes)), /*KOKKOS_LAMBDA*/[=,*this] __device__(const member_type team_member) {
            ...
            if (threadId < numRegions) {
              size_t interval_index = threadId / pow((T)numOfDivisionPerRegionPerDimension, (double)NDIM);
              size_t local_id = threadId % (size_t)pow((T)numOfDivisionPerRegionPerDimension, (double)NDIM);
              
              for (int dim = 0; dim < NDIM; ++dim) {
                size_t id =
                  (size_t)(local_id / pow((T)numOfDivisionPerRegionPerDimension, (T)dim)) % numOfDivisionPerRegionPerDimension;
                newRegions[numRegions * dim + threadId] = dRegions[currentNumRegions * dim + interval_index] + id * slength[dim];

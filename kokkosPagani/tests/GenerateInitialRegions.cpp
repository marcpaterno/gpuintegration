...
TEST_CASE("Unit-Volume 2D")
{
    ...
    SECTION("Total Volume of GPU regions is 1.0"){
        double gpu_volume = 0.;
        Kokkos::parallel_reduce("Sub-volume Reduction", kernel.numRegions, KOKKOS_LAMBDA(const int index, double& valueToUpdate){
    ...
    SECTION("Region Data Properly Transferred to CPU"){
        ...
        for(size_t index = 0; index < kernel.numRegions; index++){
            ...
            for(size_t dim=0; dim< NDIM; dim++){
				...
				//printf("dRegionslength(%i):%.15f\n", dim*kernel.numRegions + index, RegionsLength(dim*kernel.numRegions + index));
            }

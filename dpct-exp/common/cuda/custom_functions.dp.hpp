...
custom_reduce(T* arr, size_t size)
{
  ...
  */
  q_ct1.submit([&](sycl::handler& cgh) {
    sycl::
      accessor<T, 1, sycl::access_mode::read_write, sycl::access::target::local>
    ...
    cgh.parallel_for(
      ...
      [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
        device_custom_reduce(
          arr, size, out, item_ct1, (T*)shared_acc_ct1.get_pointer());
      });
  });
  /*
  ...
  */
  q_ct1.submit([&](sycl::handler& cgh) {
    ...
    cgh.parallel_for(
      sycl::nd_range(sycl::range(1, 1, 1024), sycl::range(1, 1, 1024)),
      [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {
        device_custom_reduce(
          out, num_blocks, out, item_ct1, (T*)shared_acc_ct1.get_pointer());
      });
  });
  sycl::free(out, q_ct1);
}
                             ...
                             sycl::nd_item<3> item_ct1)
{
  ...
  if ((item_ct1.get_local_id(2) & (warpSize - 1)) == 0) {
    /*
    DPCT1039:116: The generated code assumes that "out" points to the global
                                    ...
                                    sycl::nd_item<3> item_ct1)
{
  ...
  if ((item_ct1.get_local_id(2) & (warpSize - 1)) == 0) {
    /*
    DPCT1039:118: The generated code assumes that "out" points to the global
    memory address space. If it points to a local memory address space, replace
...
custom_inner_product_atomics(T1* arr1, T2* arr2, size_t size)
{
  ...
  /*
  DPCT1049:119: The workgroup size passed to the SYCL kernel may exceed the
  limit. To get the device limit, query info::device::max_work_group_size.
  Adjust the workgroup size if needed.
  */
  q_ct1.parallel_for(
...
warpReduceMax(T val, sycl::nd_item<3> item_ct1)
{
  ...
  for (int mask = item_ct1.get_sub_group().get_local_range().get(0) / 2;
       ...
       mask /= 2) {
    /*
    DPCT1023:120: The DPC++ sub-group does not support mask options for
...
warpReduceMin(T val, sycl::nd_item<3> item_ct1)
{
  ...
  for (int mask = item_ct1.get_sub_group().get_local_range().get(0) / 2;
       ...
       mask /= 2) {
    /*
    DPCT1023:121: The DPC++ sub-group does not support mask options for
    sycl::permute_group_by_xor.
    */
    val = sycl::min(
      sycl::permute_group_by_xor(item_ct1.get_sub_group(), val, mask), val);
...
min_max(T* input, const int size)
{
  ...
  */
  ...
  /*
  DPCT1049:124: The workgroup size passed to the SYCL kernel may exceed the
  limit. To get the device limit, query info::device::max_work_group_size.
  Adjust the workgroup size if needed.
  */
  q_ct1.submit([&](sycl::handler& cgh) {
    ...
    cgh.parallel_for(
      sycl::nd_range(sycl::range(1, 1, std::max(num_blocks, (size_t)32)),
  ...
  */
  ...
  if (cpy_idx < numElems) {
    ...
    if (cpy_idx + item_ct1.get_local_range().get(2) < numElems)
      s_out[item_ct1.get_local_id(2) + item_ct1.get_local_range().get(2)] =
                   ...
                   sycl::nd_item<3> item_ct1)
{
  ...
  unsigned int d_block_sum_val = d_block_sums[item_ct1.get_group(2)];
  ...
  if (cpy_idx < numElems) {
    ...
    if (cpy_idx + item_ct1.get_local_range().get(2) < numElems)
      d_out[cpy_idx + item_ct1.get_local_range().get(2)] =
        d_in[cpy_idx + item_ct1.get_local_range().get(2)] + d_block_sum_val;
  }
            ...
            uint8_t *dpct_local)
{
  ...
  */
  ...
  unsigned int cpy_idx =
    max_elems_per_block * item_ct1.get_group(2) + item_ct1.get_local_id(2);
  if (cpy_idx < len) {
                  ...
                  const size_t numElems)
{
  ...
  // Zero out d_out
  /*
  DPCT1003:136: Migrated API does not return error code. (*, 0) is inserted. You
  may need to rewrite this code.
  */
  ((q_ct1.memset(d_out, 0, numElems * sizeof(unsigned int)).wait(), 0));
  ...
  int* d_block_sums;
  /*
  DPCT1003:137: Migrated API does not return error code. (*, 0) is inserted. You
  may need to rewrite this code.
  */
  ((d_block_sums = sycl::malloc_device<int>(grid_sz, q_ct1), 0));
  /*
  DPCT1003:138: Migrated API does not return error code. (*, 0) is inserted. You
  may need to rewrite this code.
  */
  ((q_ct1.memset(d_block_sums, 0, sizeof(int) * grid_sz).wait(), 0));
  ...
  */
  q_ct1.submit([&](sycl::handler& cgh) {
    /*
    DPCT1083:169: The size of local memory in the migrated code may be
    ...
    */
    sycl::accessor<uint8_t,
                   ...
                   sycl::access_mode::read_write,
                   sycl::access::target::local>
      dpct_local_acc_ct1(sycl::range(sizeof(int) * shmem_sz), cgh);

    cgh.parallel_for(
      sycl::nd_range(sycl::range(1, 1, grid_sz) * sycl::range(1, 1, block_sz),
  ...
  if (grid_sz <= max_elems_per_block) {
    ...
    */
    ((d_dummy_blocks_sums = sycl::malloc_device<int>(1, q_ct1), 0));
    /*
    DPCT1003:141: Migrated API does not return error code. (*, 0) is inserted.
    You may need to rewrite this code.
    */
    ((q_ct1.memset(d_dummy_blocks_sums, 0, sizeof(int)).wait(), 0));
    // gpu_sum_scan_blelloch<<<1, block_sz, sizeof(unsigned int) *
    // max_elems_per_block>>>(d_block_sums, d_block_sums, d_dummy_blocks_sums,
    // grid_sz);
    /*
    DPCT1049:142: The workgroup size passed to the SYCL kernel may exceed the
    limit. To get the device limit, query info::device::max_work_group_size.
    Adjust the workgroup size if needed.
    */
    q_ct1.submit([&](sycl::handler& cgh) {
      /*
      DPCT1083:170: The size of local memory in the migrated code may be
      ...
      */
      ...
      cgh.parallel_for(sycl::nd_range(sycl::range(1, 1, block_sz),
                                      sycl::range(1, 1, block_sz)),
                       [=](sycl::nd_item<3> item_ct1) {
                         gpu_prescan(d_block_sums,
                                     d_block_sums,
                                     d_dummy_blocks_sums,
                                     grid_sz,
                                     shmem_sz,
    ...
    You may need to rewrite this code.
    */
    ((sycl::free(d_dummy_blocks_sums, q_ct1), 0));
  }
  ...
  //  for the block sums
    ...
    You may need to rewrite this code.
    */
    ((d_in_block_sums = sycl::malloc_device<int>(grid_sz, q_ct1), 0));
    /*
    DPCT1003:145: Migrated API does not return error code. (*, 0) is inserted.
    You may need to rewrite this code.
    */
    ((q_ct1.memcpy(d_in_block_sums, d_block_sums, sizeof(int) * grid_sz).wait(),
      0));
    ...
    You may need to rewrite this code.
    */
    ((sycl::free(d_in_block_sums, q_ct1), 0));
  }
  ...
  /*
  DPCT1049:147: The workgroup size passed to the SYCL kernel may exceed the
  limit. To get the device limit, query info::device::max_work_group_size.
  Adjust the workgroup size if needed.
  */
  q_ct1.parallel_for(
    sycl::nd_range(sycl::range(1, 1, grid_sz) * sycl::range(1, 1, block_sz),
                   sycl::range(1, 1, block_sz)),
    [=](sycl::nd_item<3> item_ct1) {
      gpu_add_block_sums(d_out, d_out, d_block_sums, numElems, item_ct1);
    });

  /*
  DPCT1003:148: Migrated API does not return error code. (*, 0) is inserted. You
  may need to rewrite this code.
  */

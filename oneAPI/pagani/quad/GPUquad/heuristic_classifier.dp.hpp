...
public:
  ...
  Classification_res(quad::Range<double> some_range)
    : threshold_range(some_range)
  {}
...
public:
  ...
  size_t
  device_mem_required_for_full_split(size_t num_regions)
  {
  ...
  enough_mem_for_next_split(const size_t num_regions)
  {
    return free_device_mem(num_regions, ndim) >
           device_mem_required_for_full_split(num_regions, ndim);
  }
  ...
  need_further_classification(const size_t num_regions)
  {
    if (estimate_converged() == false ||
        enough_mem_for_next_split(num_regions, ndim) == true)
      return false;
    return true;
  }
                  ...
                  const size_t num_regions)
  {
    ...
    res.num_active = static_cast<size_t>(
      reduction<double, use_custom>(res.active_flags, num_regions));
                        ...
                        const double max_percent_err_budget)
  {
    ...
    // active_flags, num_regions):"<<dot_product<double, double,

  ...
  bool relerr_classification = true)
{
  ...
  // q_ct1.memcpy(ests, current_iter_raw_integ_estimates,
  // sizeof(double)*num_regions); q_ct1.memcpy(errs,
  // new_two_level_errorestimates, sizeof(double)*num_regions);

  // if(num_regions == 128)
  // for(int i=0; i < num_regions; ++i)
  //   std::cout<< "two-level err:"<< errs[i] << " status:"<<t_act[i] <<
  //   std::endl;
  ...
  bool relerr_classification = true)
{
  ...
  q_ct1
    .parallel_for(sycl::nd_range(sycl::range(1, 1, numBlocks) *
                                   sycl::range(1, 1, block_size),
                                 sycl::range(1, 1, block_size)),
                  [=](sycl::nd_item<3> item_ct1) {
                    quad::RefineError<double>(

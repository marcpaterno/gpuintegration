...
public:
  ...
  Region_estimates(size_t num_regions) { device_init(num_regions); }

  Region_estimates(const Region_estimates<ndim>& other)
  {
    device_init(other.size);
    quad::cuda_memcpy_device_to_device<double>(
      integral_estimates, other.integral_estimates, other.size);
    quad::cuda_memcpy_device_to_device<double>(
      error_estimates, other.error_estimates, other.size);
  }

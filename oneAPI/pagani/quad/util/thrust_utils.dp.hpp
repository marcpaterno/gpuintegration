...
reduction(T* arr, size_t size){
  ...
  if constexpr(use_custom == false ){
        ...
        T res = oneapi::dpl::experimental::reduce_async(oneapi::dpl::execution::dpcpp_default, arr, arr + size).get();
        return res;
    }
...
exclusive_scan(T* arr, size_t size, T* out){
  if constexpr(use_custom == false ){
                //dpct::device_ext& dev_ct1 = dpct::get_current_device();
                //auto q_ct1 = dev_ct1.default_queue();                    
                //oneapi::dpl::experimental::exclusive_scan_async(oneapi::dpl::execution::make_device_policy(q_ct1), arr, arr + size, out, 0.).wait();

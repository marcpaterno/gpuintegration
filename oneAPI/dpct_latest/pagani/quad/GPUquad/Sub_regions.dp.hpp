     ...
     */
        dpct::get_default_queue().submit([&](sycl::handler& cgh) {
            ...
            cgh.parallel_for(sycl::range<1>(num_starting_regions),
             ...
             {
                for(size_t dim = 0; dim < ndim; ++dim){
                   int _id = (int)(reg /  (int)pow(numOfDivisionPerRegionPerDimension, dim)) % numOfDivisionPerRegionPerDimension;
                   dLeftCoord_ct1[num_starting_regions * dim + (reg)] = (double)(_id) * starting_axis_length;  

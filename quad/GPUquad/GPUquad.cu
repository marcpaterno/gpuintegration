...
#define TAG  0
    ...
    void MPI_CLIENT_receiveQuadratureData(int nodeRank, int index, T *numInfo){//numCUDADevices, unsigned long int numRegions){
	...
	errorFlag = thkernel->IntegrateSecondPhase(epsrel, epsabs, th_integral, th_error, th_nregions, th_neval, optionalInfo);
    ...
    int MPI_MASTER_integrateSecondPhase(GPUKernelCuhre<T> *thkernel, unsigned long int numCUDADevices, T &integral, T &error, size_t &nregions, size_t &neval, T *optionalInfo){
      int devCount = 0;
      cudaGetDeviceCount(&devCount);
	...
	log << "\nNode " << nodeRank << "\nData start index : " << startIndex <<"\nData end index : " << endIndex << "\nNumber of regions per node : " << numRegionsPerNode << std::endl;
      ...
      timer::start_timer(&timer_node);
#endif
      int errorFlag = thkernel->IntegrateSecondPhase(epsrel, epsabs, integral, error, nregions, neval, optionalInfo);
    ...
    T MPI_MASTER_integrateFirstPhase(GPUKernelCuhre<T> *thkernel, T epsrel, T epsabs, T &integral, T &error, size_t &nregions, size_t &neval){
	...
	Print(msg);
      ...
      thkernel->IntegrateFirstPhase(epsrel, epsabs, integral, error, nregions, neval);
    ...
    int MPI_INTEGRATE(T epsrel, T epsabs, T &integral, T &error, size_t &nregions, size_t &neval){
      ...
      if(id == 0){
		...
		T firstPhaseTime = 0;
		firstPhaseTime = MPI_MASTER_integrateFirstPhase(thkernel, epsrel, epsabs, integral, error, nregions, neval);
		...
		T *optionalInfo = (T *)malloc(sizeof(T) * 2);
		if(thkernel->getNumActiveRegions() > 0){
		  errorFlag = MPI_MASTER_integrateSecondPhase(thkernel, numCUDADevices, integral, error, nregions, neval, optionalInfo);
		}
	...
	MPI_Finalize(); 
		...
		if(activeCUDADevicesIdx[id]!=-1){
		  ...
		  MPI_CLIENT_receiveQuadratureData(id, activeCUDADevicesIdx[id], numInfo);
		}
    ...
    int integrate(T epsrel, T epsabs, T &integral, T &error, size_t &nregions, size_t &neval){
      this->epsrel = epsrel;
      this->epsabs = epsabs;
      ...
      if(numprocs > 1){
		   MPI_Init(&argc,&argv);
			MPI_Comm_size(MPI_COMM_WORLD,&numprocs);
			errorFlag = MPI_INTEGRATE(epsrel, epsabs, integral, error, nregions, neval);
			MPI_Finalize(); 
      }
	  else{

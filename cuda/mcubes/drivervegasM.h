#ifndef DRIVER_VEGAS_M_HH
#define DRIVER_VEGAS_M_HH
...
namespace NRC {
  ...
  */
  ...
  void
  rebin(float rc, int nd, float r[], float xin[], float xi[])
  ...
  if (init <= 2) {
    ...
    if (nd != ndo) {
      for (i = 1; i <= IMAX(nd, ndo); i++)
        r[i] = 1.0;
      for (j = 1; j <= ndim; j++)
        NRC::rebin(ndo / xnd, nd, r, xin, xi[j]);
      ndo = nd;
    }
  ...
  for (it = 1; it <= itmx; it++) {
    ...
    *sd = sqrt(1.0 / swgt);
    ...
    for (j = 1; j <= ndim; j++) {
      ...
      NRC::rebin(rc / xnd, nd, r, xin, xi[j]);
    }
  }
}
...
template <typename IntegT, int NDIM>
cuhreResult<double>
vegas_book_integrate(IntegT integrand,
                     int ndim,
                     double epsrel,
                     double epsabs,
                     unsigned long ncall,
                     ...
                     int itmx)
{
  ...
  do {
    ...
    errorest = 0.;
    chi_sq = 0.;
    vegas_nrc<IntegT, NDIM>(integrand,
                            regn,
                            NDIM,
                            init,
                            ncall,
                            ...
                            &errorest,
                            &chi_sq);
    status = abs(result.errorest / result.estimate) <= epsrel ||
             result.errorest <= epsabs;

...
//no stratified samples, we don't care about intervals and sub-cubes, just have each thread draw randomly from a bin
template<typename IntegT, int ndim, typename GeneratorType = Curand_generator, size_t num_bins>
__global__
cuda_generate_random_points(unsigned int seed_init, 
    ...
    if(tid < num_random_points){
        ...
        size_t bin_ids[ndim + 1] = {0.}; //+1 because all mcubes code ignores index 0
        ...
        for(int j=1; j <= ndim; j++){
            ...
            const size_t random_bin = static_cast<size_t>(ran00*num_bins) + 1; //+1 because there is no zero bin
...
class Vegas_state{
 ...
 std::vector<Eval_Point<ndim>> Vegas_state::generate_random_points(size_t num_points){
 ...
 /*
    This function will complete some mcubes iterations and generate a state consisting of the bin coordinates
    Based on those bin coordinates, we can then execute another function to return a set of random points and/or function evaluations
*/
template <typename IntegT, int ndim, bool DEBUG_MCUBES = false, typename GeneratorType = typename::Curand_generator>
Vegas_state<ndim>
Generate_state(IntegT integrand, double ncall, int titer, quad::Volume<double, ndim> const* vol){

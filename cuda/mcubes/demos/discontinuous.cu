...
void
discontinuous_mcubes_time_and_call(std::string integ_id,
                                   double epsrel,
                                   VegasParams params,
                                   std::ostream& outfile,
...
main()
{
  std::ofstream outfile("cuda_mcubes_discontinuous.csv");
  int titer = 300;
  ...
  for (auto volume : volumes) {
    ...
    for (double ncall : required_ncall) {
      ...
      for (double epsrel : epsrels) {
        ...
        discontinuous_mcubes_time_and_call<F_6_8D_alt, ndim, num_runs>(

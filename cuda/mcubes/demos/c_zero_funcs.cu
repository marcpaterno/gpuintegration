...
void
czero_mcubes_time_and_call(std::string integ_id,
                           double epsrel,
                           VegasParams params,
                           std::ostream& outfile,
  ...
  for (auto sharpness : sharpness_params) {
    ...
    do {
      ...
      std::cout << integ_id << "," << std::scientific << ndim << ","
                << vol.lows[0] << "," << vol.highs[0] << "," << sharpness << ","
...
main()
{
  std::ofstream outfile("cuda_mcubes_czero.csv");
  int titer = 300;
  ...
  for (auto volume : volumes) {
    ...
    for (double ncall : required_ncall) {
      ...
      for (double epsrel : epsrels) {
        ...
        czero_mcubes_time_and_call<F_5_8D_alt, ndim, num_runs>(
          "f5_alt", epsrel, params, outfile, vol);
      }
    }
    ...
    for (double ncall : required_ncall) {
      ...
      for (double epsrel : epsrels) {
        ...
        quad::Volume<double, ndim> vol(volume.first, volume.second);
        czero_mcubes_time_and_call<F_5_7D_alt, ndim, num_runs>(
          "f5_alt", epsrel, params, outfile, vol);
      }
    }
    ...
    for (double ncall : required_ncall) {
      ...
      for (double epsrel : epsrels) {
        ...
        quad::Volume<double, ndim> vol(volume.first, volume.second);
        czero_mcubes_time_and_call<F_5_6D_alt, ndim, num_runs>(
          "f5_alt", epsrel, params, outfile, vol);
      }
    }
    ...
    for (double ncall : required_ncall) {
      ...
      for (double epsrel : epsrels) {
        ...
        quad::Volume<double, ndim> vol(volume.first, volume.second);
        czero_mcubes_time_and_call<F_5_5D_alt, ndim, num_runs>(
          "f5_alt", epsrel, params, outfile, vol);
      }
    }
  }

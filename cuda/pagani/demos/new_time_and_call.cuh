...
template <typename F, typename T, int ndim, bool use_custom = false, int debug = 0>
bool
                 ...
                 bool relerr_classification = true)
{
  ...
  for(int i=0; i < 11; i++){
	...
	cuhreResult<T> result = workspace.template integrate<F, predict_split, collect_iters, debug>(integrand, sub_regions, epsrel, epsabs, vol, relerr_classification);

...
void
gaussian_time_and_call(std::string id,
                       double epsrel,
                       std::ostream& outfile,
...
main()
{
  ...
  std::ofstream outfile("cuda_pagani_gaussians.csv");
  ...
  for (auto volume : volumes) {
    ...
    for (double epsrel : epsrels) {
      ...
      gaussian_time_and_call<F_4_8D_alt, ndim, use_custom, debug, num_runs>(
        "f_4_alt", epsrel, outfile, vol);
    }

    for (double epsrel : epsrels) {
      constexpr int ndim = 7;
      quad::Volume<double, ndim> vol(volume.first, volume.second);
      gaussian_time_and_call<F_4_7D_alt, ndim, use_custom, debug, num_runs>(
        "f_4_alt", epsrel, outfile, vol);
    }
    ...
    for (double epsrel : epsrels) {
      ...
      quad::Volume<double, ndim> vol(volume.first, volume.second);
      gaussian_time_and_call<F_4_6D_alt, ndim, use_custom, debug, num_runs>(
        "f_4_alt", epsrel, outfile, vol);
    }
    ...
    for (double epsrel : epsrels) {
      ...
      quad::Volume<double, ndim> vol(volume.first, volume.second);
      gaussian_time_and_call<F_4_5D_alt, ndim, use_custom, debug, num_runs>(
        "f_4_alt", epsrel, outfile, vol);
    }
  }

...
namespace detail {
  ...
  public:
    ...
    {
      return exp(-1.0 * (pow(alphas[0], 2) * pow(x - betas[0], 2) +
...
integrate_gaussians()
{
  ...
  // double epsrel = 1.e-3;
  // original values
  std::array<double, ndim> alphas = {
    25., 25., 25., 25., 25.}; // alpha is difficulty param
  std::array<double, ndim> betas = {.5, .5, .5, .5, .5};
  Config configuration;
  quad::Volume<double, ndim> vol;

  std::cout << "id, alg, difficulty, epsrel, epsabs, integral, estimate, "
               "errorest, time, status\n";
  ...
  for (double epsrel = 1.e-3; epsrel > 1.e-9; epsrel /= 5.) {
    ...
    for (double difficulty = 0.; difficulty < 50; difficulty += 1.) {
      ...
      common_header_pagani_time_and_call<detail::Gaussian, ndim>("pagani",
                                                                 "f4 5D",
                                                                 integrand,
                                                                 epsrel,
                                                                 true_value,
      ...
      common_header_mcubes_time_and_call<detail::Gaussian, ndim>(integrand,
                                                                 epsrel,
                                                                 true_value,
                                                                 difficulty,
                                                                 "mcubes",
...
void integrate_product_peaks(){
    ...
    std::array<double, ndim> alphas = {25., 25., 25., 25., 25.};//alpha is
difficulty param std::array<double, ndim> betas = {.5, .5, .5, .5, .5};
}*/

...
int main(){
    ...
    double* generators = cuda_malloc<double>(sizeof(double) * ndim * fEvalPerRegion);
	...
	quad::VEGAS_ASSISTED_INTEGRATE_GPU_PHASE1<GENZ_4_2D, double, ndim, 64><<<sub_regions.size, 64>>>(d_integrand,
 ...
 double mcubes_est =
  ...
  double mcubes_err =
	...
	quad::INTEGRATE_GPU_PHASE1<GENZ_4_2D, double, ndim, 64><<<sub_regions.size, 64>>>(d_integrand,
            sub_regions.dLeftCoord, 
            sub_regions.dLength,
			sub_regions.size, 
            estimates.integral_estimates,

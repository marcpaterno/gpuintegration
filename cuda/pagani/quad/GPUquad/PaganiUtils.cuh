...
class Cubature_rules{
    ...
    Cubature_rules(){
        ...
        quad::ComputeGenerators<T, ndim><<<1, block_size>>>(generators, fEvalPerRegion, constMem);
	...
	void
	Print_func_evals(quad::Func_Evals<ndim> fevals, T* ests, T* errs, const size_t num_regions){
		if(num_regions >= 1024)
			return;
	...
	void print_generators(T* d_generators){
		rgenerators.outfile << "i, gen" << std::endl;
		T* h_generators = new T[ndim * pagani::CuhreFuncEvalsPerRegion<ndim>()];
		cuda_memcpy_to_host<T>(h_generators, d_generators, ndim * pagani::CuhreFuncEvalsPerRegion<ndim>());
	...
	void print_verbose(T* d_generators, quad::Func_Evals<ndim>& dfevals, const Reg_estimates& estimates){
		...
		if constexpr(debug >= 1){
			...
			cuda_memcpy_to_host<T>(ests, estimates.integral_estimates, num_regions);
			cuda_memcpy_to_host<T>(errs, estimates.error_estimates, num_regions);
    ...
    void Setup_cubature_integration_rules(){
        ...
        size_t block_size = 64;
        quad::ComputeGenerators<T, dim><<<1, block_size>>>(generators, fEvalPerRegion, constMem);
    }
...
template<typename T, size_t ndim, bool use_custom = false>
cuhreResult<T>
compute_finished_estimates(const Region_estimates<ndim>& estimates, const Region_characteristics<ndim>& classifiers, const cuhreResult<T>& iter){
    cuhreResult<T> finished; 
	finished.estimate = iter.estimate - dot_product<int, T, use_custom>(classifiers.active_regions, estimates.integral_estimates, estimates.size);;
    finished.errorest = iter.errorest - dot_product<int, T, use_custom>(classifiers.active_regions, estimates.error_estimates, estimates.size);;
	return finished;
}

template<typename T>
bool accuracy_reached(T epsrel, T epsabs, T estimate, T errorest){
...
template<typename T>
bool accuracy_reached(T epsrel, T epsabs, cuhreResult<T> res){
...
cuhreResult<T>
pagani_clone(const IntegT& integrand, Sub_regions<ndim>& subregions, T epsrel = 1.e-3, T epsabs = 1.e-12, bool relerr_classification = true){
    using Reg_estimates = Region_estimates<ndim>;
    using Sub_regs = Sub_regions<ndim>;
    using Regs_characteristics = Region_characteristics<ndim>;
    ...
    for(size_t it = 0; it < 700 && !accuracy_termination; it++){
        ...
        if(!accuracy_termination){
            ...
            Res finished;
            finished.estimate = iter.estimate - dot_product<int, T>(classifiers.active_regions, estimates.integral_estimates, num_regions);
            finished.errorest = iter.errorest - dot_product<int, T>(classifiers.active_regions, estimates.error_estimates, num_regions);

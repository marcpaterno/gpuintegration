...
private:
  void fix_error_budget_overflow(Region_characteristics<ndim>& classifiers,
                                 const cuhreResult<double>& finished,
  ...
  const cuhreResult<double>& cummulative)
{
  ...
  if (hs_classify_success) {
    ...
    finished.estimate = iter.estimate - dot_product<int, double, use_custom>(
                                       ...
                                       bool relerr_classification)
{
  ...
  for (size_t it = 0; it < 700 && subregions.size > 0; it++) {
    ...
    Res finished = compute_finished_estimates<ndim, use_custom>(

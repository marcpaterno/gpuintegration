...
public:
  ...
  template<typename IntegT, bool predict_split = false, bool collect_iters = false, int debug = 0>
  cuhreResult<double> integrate(const IntegT& integrand, double epsrel, double epsabs, quad::Volume<double, ndim>& vol, bool relerr_classification = true);
};
...
template<typename IntegT, bool predict_split, bool collect_iters, int debug>
cuhreResult<double>       
Workspace<ndim, use_custom>::integrate(const IntegT& integrand, double epsrel, double epsabs, quad::Volume<double, ndim>& vol, bool relerr_classification){
  using MilliSeconds = std::chrono::duration<double, std::chrono::milliseconds::period>;

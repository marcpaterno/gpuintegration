...
#pragma unroll 9
    ...
    for (size_t pass = 0; pass < num_passes; ++pass) {
      ...
      for (size_t sample = 0; sample < num_samples; ++sample) {
        ...
        for (int dim = 1; dim <= NDIM; ++dim) {
          ...
          // get the true bounds
          const double region_scaled_b_high =
            scale_point(bin_high, b[dim - 1].lower, b[dim - 1].upper);
          const double region_scaled_b_low =
            scale_point(bin_low, b[dim - 1].lower, b[dim - 1].upper);

          // scale a random point at those bounds

          double rand_point_in_bin = scale_point(
            (rand_num_generator)(), region_scaled_b_low, region_scaled_b_high);

          /*if(threadIdx.x == 0 && blockIdx.x == 15)
                  printf("sample %lu bin %i dim %i scaled bin bounds:%f,%f
             point:%e\n", sample, bin, dim, region_scaled_b_low,
             region_scaled_b_high, rand_point_in_bin);*/
        ...
        /*if(f != 0.  && blockIdx.x == 0){
                printf("\t\t feval tid:%i f:%e, bins:(%i,%i) contr
        indices:(%i,%i) %f, %f\n", threadIdx.x, f, bins[1], bins[2], bins[1] *
        (NDIM+1) + 1, bins[2] * (NDIM+1) + 2, x_random[0], x_random[1]);
        }*/
        ...
        for (int dim = 1; dim <= NDIM; ++dim) {
          ...
          /*
          if(f != 0.)
                  printf("tid %i dim:%i bin:%i adding %e (f:%e) to index
          d[%i]\n", threadIdx.x, dim, bins[dim], pass_sq_sum, pass_sum,
          bins[dim] * (NDIM+1) + dim);*/
        }
      }
      ...
      if (threadIdx.x == 0) {
        ...
        dim, bin, d[(bin)*(NDIM+1) + dim], scale_point(xi[(dim)*(nbins+1) + bin
        -1], b[dim-1].lower, b[dim-1].upper), scale_point(xi[(dim)*(nbins+1) +
        bin], b[dim-1].lower, b[dim-1].upper));
                        }
                        printf("---------------------------------\n");
                }
                printf("========================\n");
                                   ...
                                   unsigned int seed_init)
  {
    ...
    /*if(threadIdx.x == 0)
            printf("region %i bounds: (%f - %f) (%f - %f)  (%f - %f)  (%f - %f)
       (%f - %f)\n", blockIdx.x, region->bounds[0].lower,
       region->bounds[0].upper, region->bounds[1].lower,
       region->bounds[1].upper, region->bounds[2].lower,
       region->bounds[2].upper, region->bounds[3].lower,
       region->bounds[3].upper, region->bounds[4].lower,
       region->bounds[4].upper);*/
...
#pragma unroll 9
    ...
    if (threadIdx.x == 0) {
      Result* r = &region->result;
      double mean =
        ran_sum / static_cast<double>(64 * num_samples * num_passes);
      double var = sq_sum / static_cast<double>(64 * num_passes * num_samples) -
                   mean * mean;
      // printf("sum:%e (squared:%e) sq_sum:%e\n", ran_sum, ran_sum*ran_sum,
      // sq_sum);

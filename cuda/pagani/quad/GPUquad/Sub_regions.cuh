                     ...
                     size_t ndim)
{
    ...
    if (threadId < newNumOfRegions) {
      size_t interval_index = threadId / pow((double)numOfDivisionsPerRegionPerDimension, (double)ndim);
      size_t local_id =  threadId % (size_t)pow((double)numOfDivisionsPerRegionPerDimension, (double)ndim);
      for (int dim = 0; dim < ndim; ++dim) {
        size_t id = (size_t)(local_id /  pow((double)numOfDivisionsPerRegionPerDimension, (double)dim)) % numOfDivisionsPerRegionPerDimension;
...
struct Sub_regions{
  ...
  void device_init(size_t const numRegions){
	cudaFree(dLeftCoord);
	cudaFree(dLength);
	size = numRegions;
    dLeftCoord = cuda_malloc<double>(numRegions*ndim);  
  ...
  void uniform_split(size_t numOfDivisionPerRegionPerDimension){
    ...
    create_uniform_split<<<numBlocks, numThreads>>>(starting_axis_length, dLeftCoord, dLength, num_starting_regions, numOfDivisionPerRegionPerDimension, ndim);

...
struct Range{
 ...
 template<typename T>
  void set_device_array_range(T* arr, size_t first_to_change, size_t last_to_change, size_t  size, T val){
    ...
    set_array_range_to_value<T><<<num_blocks, num_threads>>>(arr, first_to_change, last_to_change, size, val);
    cudaDeviceSynchronize();
  }   
...
array_values_smaller_than_val(T* dev_arr, size_t dev_arr_size, C val){
    ...
    cudaMemcpy(host_arr, dev_arr, sizeof(double)*dev_arr_size, cudaMemcpyDeviceToHost); 

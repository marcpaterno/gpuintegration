...
namespace gpu {
  ...
  public:
    ...
    __device__ T& operator[](std::size_t i) { return data[i]; }

    __device__ T const& operator[](std::size_t i) const
    {
      return data[i];
    }
...
class Internal_Vegas_Params{
    ...
    public:
        ...
        static constexpr int get_NDMX(){return ndmx;}
   
        static constexpr int get_NDMX_p1(){return ndmx+1;}
       
        static constexpr  double get_ALPH(){return alph;}
       
        static constexpr  int get_MXDIM(){return mxdim;}
        
        constexpr static int get_MXDIM_p1(){return mxdim+1;}
};

__inline__  __host__ __device__
void get_indx(int ms, uint32_t *da, int ND, int NINTV) {
...
class Curand_generator{
    ...
    public:
        __device__ Curand_generator(int blockId, int threadId){
            #if defined(__CUDA_ARCH__) 
            curand_init(0, blockId, threadId, &localState);
            #endif
        }
        
        __device__ Curand_generator(unsigned int seed, int blockId, int threadId){
            #if defined(__CUDA_ARCH__) 
...
class Random_num_generator{
    ...
    public:
        ...
        __device__ Random_num_generator(unsigned int seed, int blockId, int threadId):generator(seed, blockId, threadId){}
...
__device__ void Setup_Integrand_Eval(Random_num_generator<GeneratorType>* rand_num_generator,
                            double xnd, 
                            double dxg, 
                            ViewVectorDouble xi, 
...
Process_npg_samples(Kokkos::View<IntegT*, Kokkos::CudaUVMSpace> integrand, 
      ...
      for (int k = 1; k <= npg; k++) {
        ...
        Setup_Integrand_Eval<ndim, GeneratorType>(rand_num_generator, 
            xnd, dxg, xi, regn, dx, kg,  ia, x, wgt, npg, k, cube_id);
...
template <typename IntegT, int ndim, typename GeneratorType = Curand_generator>
__device__ void Process_chunks(Kokkos::View<IntegT*, Kokkos::CudaUVMSpace> integrand, 
                int chunkSize, 
                int lastChunk, 
                int ng, 
                int npg, 
                ...
                size_t cube_id_offset)
{
    ...
    for (int t = 0; t < chunkSize; t++) {
      ...
      Process_npg_samples<IntegT, ndim, GeneratorType>(integrand, npg, xnd, xjac, rand_num_generator, dxg, regn, dx, xi, kg, ia, x, wgt, d, fb, f2b,  cube_id);
...
template <typename IntegT, int ndim, typename GeneratorType = Curand_generator>
void vegas_kernel_kokkos(Kokkos::View<IntegT*, Kokkos::CudaUVMSpace> integrand, 
                        uint32_t nBlocks, 
                        uint32_t nThreads, 
                        int ng, 
                        int npg, 
	...
	Kokkos::parallel_for( "vegas_kernel",
	                      team_policy(nBlocks, nThreads).set_scratch_size( 0, Kokkos::PerTeam(2048)),
						  KOKKOS_LAMBDA ( const member_type team_member) 
	{
        int chunkSize = _chunkSize;
		ScratchViewDouble sh_buff(team_member.team_scratch(0), 2*BLOCK_DIM_X*sizeof(double));
        
		constexpr int mxdim_p1 = Internal_Vegas_Params::get_MXDIM_p1();
		uint32_t tx = team_member.team_rank ();  //local id
		uint32_t m = team_member.league_rank () * BLOCK_DIM_X + tx;  //global thread id
		double  wgt;
		...
		if (m < totalNumThreads) {
            ...
            size_t cube_id_offset = (team_member.league_rank () * BLOCK_DIM_X + tx)*chunkSize;
			
			if (m == totalNumThreads - 1) 
                chunkSize = LastChunk;
            
            Random_num_generator<GeneratorType> rand_num_generator(seed_init, team_member.league_rank(), team_member.team_rank());
...
template <typename IntegT, int ndim, typename GeneratorType = Curand_generator>
void vegas_kernel_kokkosF(Kokkos::View<IntegT*, Kokkos::CudaUVMSpace> integrand, 
                        uint32_t nBlocks, 
                        uint32_t nThreads, 
                        int ng, 
                        int npg, 
...
template <typename IntegT, int ndim, typename GeneratorType = typename::Curand_generator>
void vegas(IntegT integrand,
            double epsrel,
            double epsabs,
            double ncall,
            double *tgral,
            ...
            Volume<double, ndim> const* vol)
{
	
	{
		...
		ViewVectorDouble d_xi( "xi", ((ndmx_p1) * (mxdim_p1)) );//xi_dev in the original  
		ViewVectorDouble d_d( "d", ((ndmx_p1) * (mxdim_p1)) );   //d_dev in the original
                ...
                *status = GetStatus(*tgral, *sd, it, epsrel, epsabs);
                ...
                for (j = 1; j <= ndim; j++) {
                  if (dt[j] > 0.0) { // enter if there is any contribution only
                    ...
                    rebin(rc / xnd, nd, r, xin, xi.data()+(j * ndmx_p1));  //first bin of each dimension is at a diff index
                  }
                }
        }

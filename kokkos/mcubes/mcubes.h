...
namespace kokkos_mcubes{
	...
	public:
	  ...
	  operator()()
	  {
		...
		//printf("random %f a:%lu c:%lu temp:%lu p:%lu\n", (double)custom_seed / (double)p, a, c, temp, p);
	...
	template <typename IntegT, int ndim, typename GeneratorType = kokkos_mcubes::Custom_generator>
	KOKKOS_INLINE_FUNCTION void
				   ...
				   size_t cube_id_offset)
	{
	  ...
	  for (int t = 0; t < chunkSize; t++) {
		...
		if constexpr (kokkos_mcubes::TypeChecker<GeneratorType, Custom_generator>::
						is_custom_generator()) {
	  ...
	  Kokkos::parallel_for(
		...
		KOKKOS_LAMBDA(const member_type team_member) {
		  ...
		  if (m < totalNumThreads) {
			...
			size_t cube_id_offset =
			  (team_member.league_rank() * team_member.team_size() + tx) * chunkSize;
		  ...
		  if (tx == 0) {
			//double fbgs = 0.0;
			//double f2bgs = 0.0;
			/*for (int ii = 0; ii < Internal_Vegas_Params::get_NDMX() && (m + ii) < totalNumThreads; ++ii) {
			  fbgs += sh_buff(ii);
			  f2bgs += sh_buff(ii + Internal_Vegas_Params::get_NDMX());
			}*/
			//printf("block %i storing %f +- %f\n", team_member.league_rank(), fbg, f2bg);
						 ...
						 unsigned int seed_init)
	{
	  ...
	  Kokkos::parallel_for(
		...
		KOKKOS_LAMBDA(const member_type team_member) {
		  ...
		  uint32_t m =
			team_member.league_rank() * Internal_Vegas_Params::get_NDMX() + tx; // global thread id
		  ...
		  if (m < totalNumThreads) {
			...
			size_t cube_id_offset =
			  (team_member.league_rank() * Internal_Vegas_Params::get_NDMX() + tx) * chunkSize;

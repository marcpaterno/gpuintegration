...
namespace quad {
  ...
  class Interp3D {
    ...
    Alloc(size_t x, size_t y, size_t z)
    {
      if (x > 100000 || y > 100000 || z > 100000) {
        std::cerr << "InterpD::Alloc called with x=" << x
                  << " and y=" << y << " and z=" << z << '\n';
        std::abort();
      }
      if (x * y * z > 1000000) {
        std::cerr << "Interp3D::Alloc called with x=" << x
                  << " and y=" << y << " and z=" << z << '\n';
        std::abort();
      }
  ...
  public:
    ...
    Interp3D(const Interp3D& source)
    {
      ...
      cuda_memcpy_device_to_device<double>(interpT, source.interpT, size_x * size_y * size_z);
    ...
    Interp3D(std::vector<double> const& xs,
      ...
      : Interp3D(xs.data(), ys.data(), zs.data(), vs.data(), xs.size(), ys.size(), zs.size())
    {
      CudaCheckError();
    }
    ...
    operator()(double x, double y, double z) const
    {
      ...
      FindNeighbourIndices(x, _xs, size_x, x0, x1);
	    ...
	    //interpolate along x
	    const double c00 = interpT[index(x0, y0, z0)] * (1 - x_d) + interpT[index(x1,y0,z0)] * x_d;
	    const double c01 = interpT[index(x0, y0, z1)] * (1 - x_d) + interpT[index(x1, y0, z1)] * x_d;
	    const double c10 = interpT[index(x0, y1, z0)] * (1 - x_d) + interpT[index(x1, y1, z0)] * x_d;
	    const double c11 = interpT[index(x0, y1, z1)] * (1 - x_d) + interpT[index(x1, y1, z1)] * x_d;
    ...
    {
      return eval(do_clamp(x, min_x(), max_x()), do_clamp(y, min_y(), max_y()), do_clamp(z, min_z(), max_z()));
    }
  };
}

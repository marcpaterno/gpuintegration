...
namespace quad{
  ...
  public:
    ...
    GPUcuhre(int pargc, char **pargv, int key = 0, int verbose = 0, int numDevices = 1){
      argc = pargc;
      argv = pargv;
      KEY = key;
      VERBOSE = verbose;
...
#define TAG  0
    ...
    void MPI_CLIENT_receiveQuadratureData(IntegT* d_integrand, int nodeRank, int index, T *numInfo){//numCUDADevices, unsigned long int numRegions){
	...
	errorFlag = thkernel->IntegrateSecondPhase(d_integrand, epsrel, epsabs, th_integral, th_error, th_nregions, th_neval, optionalInfo);
    ...
    template <typename IntegT>
    int MPI_MASTER_integrateSecondPhase(GPUKernelCuhre<T, NDIM> *thkernel, IntegT* d_integrand, unsigned long int numCUDADevices, T &integral, T &error, size_t &nregions, size_t &neval, T *optionalInfo){
      int devCount = 0;
      cudaGetDeviceCount(&devCount);
	...
	log << "\nNode " << nodeRank << "\nData start index : " << startIndex <<"\nData end index : " << endIndex << "\nNumber of regions per node : " << numRegionsPerNode << std::endl;
      ...
      timer::start_timer(&timer_node);
#endif
      int errorFlag = thkernel->IntegrateSecondPhase(d_integrand, epsrel, epsabs, integral, error, nregions, neval, optionalInfo);
    ...
    void MPI_MASTER_sendQuadratureData(GPUKernelCuhre<T, NDIM> *thkernel, std::map<int, std::vector<std::string> > CUDANodeDetails, int numCUDADevices){
    ...
    template <typename IntegT>
    T MPI_MASTER_integrateFirstPhase(GPUKernelCuhre<T, NDIM> *thkernel, IntegT* d_integrand, T epsrel, T epsabs, T &integral, T &error, size_t &nregions, size_t &neval){
	...
	Print(msg);
      ...
      thkernel->IntegrateFirstPhase(d_integrand, epsrel, epsabs, integral, error, nregions, neval);
    ...
    template <typename IntegT>
    int MPI_INTEGRATE(IntegT* integrand, T epsrel, T epsabs, T &integral, T &error, size_t &nregions, size_t &neval){
      ...
      if(id == 0){
		...
		T firstPhaseTime = 0;
		firstPhaseTime = MPI_MASTER_integrateFirstPhase(thkernel, d_integrand, epsrel, epsabs, integral, error, nregions, neval);
		...
		T *optionalInfo = (T *)malloc(sizeof(T) * 2);
		if(thkernel->getNumActiveRegions() > 0){
		  errorFlag = MPI_MASTER_integrateSecondPhase(thkernel, d_integrand, numCUDADevices, integral, error, nregions, neval, optionalInfo);
		}
...
#endif
	printf("%d\t%e\t%.10lf\t%.10f\t%-15ld\t%-15ld\t%d\t%-10.2lf\t%-10.2lf\n", NDIM, epsrel, integral, error, nregions, neval, errorFlag, kernelTime+firstPhaseTime, mpiCommTime+totalTime);
	...
	MPI_Finalize(); 
		...
		if(activeCUDADevicesIdx[id]!=-1){
		  ...
		  if(VERBOSE)log.open(logfilename.c_str());
    ...
    int integrate(IntegT integrand, T epsrel, T epsabs, T &integral, T &error, size_t &nregions, size_t &neval, size_t& fregions, int& lastPhase, Volume<T, NDIM>* vol = nullptr){
      ...
      if(numprocs > 1){
		   MPI_Init(&argc,&argv);
			MPI_Comm_size(MPI_COMM_WORLD,&numprocs);
			errorFlag = MPI_INTEGRATE(&integrand, epsrel, epsabs, integral, error, nregions, neval);
			MPI_Finalize(); 
      }
	  else{
	...
	//printf("%d\t%e\t%.15lf\t%.15f\t%-15ld\t%-15ld\t%d\t%-10.2lf\t%-10.2lf\n", NDIM, epsrel, integral, error, nregions, neval, errorFlag);
    //printf("Error flag:%i\n", errorFlag);
	//printf("Total Time:%f\n", time);
#endif
      }      

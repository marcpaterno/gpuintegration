...
FourthDividedDifference(double* sdata, int NDIM, int* maxdim){
    ...
    for (int dim = 0; dim < NDIM; ++dim) {
        double* fp = f1 + 1;
        double* fm = fp + 1;
        double fourthdiff = fabs(base + ratio * (fp[0] + fm[0]) - (fp[offset] + fm[offset]));
        f1 = fm;
...
newFourthDividedDifference(double* sdata, int NDIM, int* maxdim){
    ...
    int bisectdim = *maxdim;
    for (int dim = 0; dim < NDIM; ++dim) {
        double fourthdiff = fabs(base + ratio * (sdata[index + 1] + sdata[index + 2]) - (sdata[index + 1 + offset] + sdata[index + 2 + offset]));
        if (fourthdiff > maxdiff) {
          maxdiff = fourthdiff;
          bisectdim = dim;
        }
...
int GetMaxDim(double* regionLows, double* regionHighs, int NDIM){
    double maxRange = 0;
    int maxDim = 0;
    for (int dim = 0; dim < NDIM; ++dim) {
...
void Fill_sdata(double* sdata, int NDIM){
...
TEST_CASE("Check Across Dimensions 2-8")
{
    ...
    for(int dim = 8; dim >= 2; dim--){
        ...
        int maxDim = GetMaxDim(lows + dim - 2, highs + dim - 2, ndim); //to avoid multiple highs, lows arrays

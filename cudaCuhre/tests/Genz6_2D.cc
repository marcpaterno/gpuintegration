...
double GENZ_6_2D(double y, double z)
{
	  if(z > .9 || y > .8 )
		  return 0.;
	  else
...
int main()
{
  ...
  double true_value = 120489.75982636053604;
   while(time_and_call_alt(cuhre, GENZ_6_2D, epsrel, true_value, "dc_f0") == true &&  epsrel >= epsrel_min)
  {
  ...
  cuhre.flags = 4;
  epsrel = 1.0e-3;
  while(time_and_call_alt(cuhre, GENZ_6_2D, epsrel, true_value, "dc_f1") == true && epsrel >= epsrel_min)
  {
      epsrel /= 5.0;
  }
  return 0;

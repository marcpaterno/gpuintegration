...
TEST_CASE("BoxIntegral8_25")
{
	...
	using MilliSeconds = std::chrono::duration<double, std::chrono::milliseconds::period>;
	auto t0 = std::chrono::high_resolution_clock::now();
	auto const result = cuhre.integrate<BoxIntegral8_25>(integrand, epsrel, epsabs, &vol, outfileVerbosity, _final, phase_I_type);
...
TEST_CASE("BoxIntegral8_22")
{
	...
	using MilliSeconds = std::chrono::duration<double, std::chrono::milliseconds::period>;
	auto t0 = std::chrono::high_resolution_clock::now();
	auto const result = cuhre.integrate<BoxIntegral8_22>(integrand, epsrel, epsabs, &vol, outfileVerbosity, _final, phase_I_type);

...
public:
  ...
  Snapshotsim_ScalarIntegrand_Sigma(cosmosis::DataBlock& config): hmf(), sigma(), radius_(), zt_()
  {}
  ...
  void set_sample(hmf_t<T> const& hmf_in, sig_sum<T> const& sig_sum_in){
    hmf = std::move(hmf_in);
  ...
  /*auto constexpr*/double simulation_cosmic_volume = 4492125./*165.0 * 165.0 * 165.0*/;
  double val = simulation_cosmic_volume*(hmf)(lnM, zt_) *

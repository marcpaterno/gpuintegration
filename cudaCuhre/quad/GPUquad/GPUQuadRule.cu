		.01553241727660705326386197156586357005224 - NDIM*.003541756451678267682601411863388846964536, .003532809960709087023561817517751309380604 - NDIM*.002148602555009868771294231899653510655506, -(.003532809960709087023561817517751309380604 - NDIM*.002148602555009868771294231899653510655506),  .09231719987444221619017126187763868745587 + .01553241727660705326386197156586357005224 - NDIM*.003541756451678267682601411863388846964536, -(.01553241727660705326386197156586357005224 - NDIM*.003541756451678267682601411863388846964536),

		.02254314464717892037990281369120402214829, .01367577326327282236101845043145111753718, -.01367577326327282236101845043145111753718, -.3254475969596012529657378160439011607639, -.02254314464717892037990281369120402214829,

		.001770878225839133841300705931694423482268, .001074301277504934385647115949826755327753, -.001074301277504934385647115949826755327753, .001770878225839133841300705931694423482268, -.001770878225839133841300705931694423482268,

  
		.2515001149531479199576969952416196054795/(1<<NDIM), -.06287502873828697998942424881040490136987/(1<<NDIM), -(-.06287502873828697998942424881040490136987/(1<<NDIM)), .2515001149531479199576969952416196054795/(1<<NDIM), -(.2515001149531479199576969952416196054795/(1<<NDIM))
      };
	  ...
	  T cpuRule9G[] = {
		.4779536579022695061928604197171830064732, .2030285873691198677998034402373279133258,
		.4476273546261781288207704806530998539285, .125,
  ...
  public:
	...
	inline int GET_FEVAL(){return FEVAL;}
	inline int GET_NSETS(){return NSETS;}
	...
	void loadDeviceConstantMemory(Structures<T> *constMem, int device = 0){
      ...
      QuadDebug(cudaMemcpy(constMem->_gpuG, 				cpuG, 				sizeof(T)*NDIM *NSETS, 						cudaMemcpyHostToDevice));
      QuadDebug(cudaMemcpy(constMem->_cRuleWt, 				CPURuleWt, 			sizeof(T)*NRULES *NSETS, 					cudaMemcpyHostToDevice));
      QuadDebug(cudaMemcpy(constMem->_cGeneratorCount, 		CPUGeneratorCount, 	sizeof(size_t)*NSETS, 						cudaMemcpyHostToDevice));
      QuadDebug(cudaMemcpy(constMem->_GPUScale, 			CPUScale, 			sizeof(T)*NSETS*NRULES, 					cudaMemcpyHostToDevice));
      QuadDebug(cudaMemcpy(constMem->_GPUNorm, 				CPUNorm, 			sizeof(T)*NSETS*NRULES, 					cudaMemcpyHostToDevice));
      QuadDebug(cudaMemcpy(constMem->_gpuGenPos, 			genPtr, 			sizeof(int)*PERMUTATIONS_POS_ARRAY_SIZE, 	cudaMemcpyHostToDevice));
      QuadDebug(cudaMemcpy(constMem->_gpuGenPermVarCount, 	cpuGenPermVarCount, sizeof(int)*FEVAL, 							cudaMemcpyHostToDevice));
      QuadDebug(cudaMemcpy(constMem->_gpuGenPermGIndex, 	cpuGenPermGIndex, 	sizeof(int)*FEVAL, 							cudaMemcpyHostToDevice));
      QuadDebug(cudaMemcpy(constMem->_gpuGenPermVarStart, 	cpuGenPermVarStart, sizeof(int)*(FEVAL+1), 						cudaMemcpyHostToDevice));
	  
    }


    void Init(int ndim, size_t fEval, int key, int verbose, Structures<T> *constMem){
      NDIM = ndim;
      KEY = key;
      VERBOSE = verbose;
	  ...
	  FEVAL = (1 + 2*DIM + 2*DIM + 2*DIM + 2*DIM + 2*DIM*(DIM - 1) + 4*DIM*(DIM - 1) + 4*DIM*(DIM - 1)*(DIM - 2)/3 + (1 << DIM));
      PERMUTATIONS_POS_ARRAY_SIZE = (1+1*1 + 2*DIM*1 + 2*DIM*1 + 2*DIM*1 + 2*DIM*1 + 2*DIM*(DIM - 1)*2 + 4*DIM*(DIM - 1)*2 + 4*DIM*(DIM - 1)*(DIM - 2)*3/3 + DIM*(1 << DIM));
	  //NRULES = 5;
	  Rule9Generate();

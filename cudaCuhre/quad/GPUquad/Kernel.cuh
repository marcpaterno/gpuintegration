...
namespace quad {
  ...
  public:
    ...
    { 
      CudaCheckError();
	  //dRegions and dRegionsLength need to be freed after phase 1, since all the info is stored in host memory
                        ...
                        int last_iteration = 0)
    {
	 ...
	 if (outLevel >= 1)
	  ...
	  //printf("Phase 1 iteration ends with %lu regions and %lu slots in dRegionsIntegral/Error\n", numRegions, numRegions*2);
	  ...
	  if (numRegions <= first_phase_maxregions && fail == 1) {
        GenerateActiveIntervals(activeRegions,
		  ...
		  //display<double>(dRegionsIntegral+numRegions, dRegionsError+numRegions, numRegions);
	  }
      QuadDebug(cudaFree(activeRegions));
      QuadDebug(cudaFree(subDividingDimension));
      QuadDebug(cudaFree(newErrs));
                              ...
                              Volume<T, NDIM>* vol = nullptr)
    {
	  ...
	  currentBatch->Set(NDIM, numCurrentRegions, lows, highs, dPh1res, dPh1res + 1 , nullptr, converged);
	...
	void Phase_I_format_region_copy(RegionList*& batch,
		...
		for (int dim = 0; dim < NDIM; ++dim) {
			//printf("Copying regions: from %lu to %lu\n", dim * total_num_regions + startRegionIndex, dim * total_num_regions + startRegionIndex+batch_size);
	...
	PhaseII_output Execute_PhaseII_Batches(double* Regions, 
		...
		for(int it = 0; it < iters; it++){
			...
			//printf("Assigning dRegionsIntegral[%lu] to batch %i\n", numRegions + leftIndex, it);
			Phase_I_format_region_copy(currentBatch, Regions, RegionsLength, dRegionsIntegral + numRegions + leftIndex, dRegionsError + numRegions + leftIndex, size, leftIndex, rightIndex); //extract the coordinate info, deep copy
			CudaCheckError();
		...
		if( size % max_num_blocks != 0){
			...
			Phase_I_format_region_copy(currentBatch, Regions, RegionsLength, dRegionsIntegral + numRegions + leftIndex, dRegionsError + numRegions + leftIndex, size, leftIndex, rightIndex);
			final_result += Execute_PhaseII_Batch(currentBatch, 
												  dRegionsNumRegion,
												  gpu_id,
												  Phase_I_result,
	...
	void Assing_Regions_To_Processor(double*& dRegionsThread, 
		...
		//it assigns all regions that will ever be processed in phase 2 by a processor
		//the batching doesn't happen here
                         ...
                         size_t& neval/*,T* optionalInfo = 0*/)
    {
	  ...
	  
      {
        ...
        if (cpu_thread_id < num_cpu_threads) {
          ...
          QuadDebug(Device.AllocateUnifiedMemory((void**)&dRegionsNumRegion, sizeof(int) * numRegionsThread));
		  
          if (phase_I_type == 0) {
			//dRegionsThread & dRegionsLengthThread store all regions and get contents (partially or fully) from hRegions and hRegionsLength
            QuadDebug(Device.AllocateUnifiedMemory((void**)&dRegionsThread, 		sizeof(T) * numRegionsThread * NDIM));
            QuadDebug(Device.AllocateUnifiedMemory((void**)&dRegionsLengthThread, 	sizeof(T) * numRegionsThread * NDIM));
			
			//this function creates a batch out of all regins, ready to be passed as an object to Phase 2 Kernel
			Assing_Regions_To_Processor(dRegionsThread, dRegionsLengthThread, numRegions, cpu_thread_id, num_cpu_threads);	
			CudaCheckError();			
          }
			
		  //RegionList* currentBatch = new RegionList(NDIM, numRegionsThread);
		  //currentBatch->Set(NDIM, numRegionsThread, dRegionsThread, dRegionsLengthThread, dRegionsIntegral, dRegionsError);
          CudaCheckError();

...
class Interp2D : public Managed{
  public:
    ...
    Interp2D(std::array<double, M> const& xs, std::array<double, N> const& ys, 
    ...
    template<size_t M, size_t N>
    Interp2D(std::array<double, M> xs, std::array<double, N> ys, 
    ...
    __device__ 
    bool AreNeighbors(const double val, double* arr, const size_t leftIndex, 
    ...
    __device__
    void FindNeighbourIndices(const double val, double* arr, const size_t size, 
        size_t& leftI, size_t& rightI) const{

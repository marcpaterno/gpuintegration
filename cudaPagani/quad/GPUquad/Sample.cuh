...
namespace quad {
                     ...
                     double* sdata)
  {
    ...
    for (int dim = 0; dim < NDIM; ++dim) {
      ...
      x[dim] = sBound[dim].unScaledLower +((.5 + generator) * b[dim].lower + (.5
  - generator) * b[dim].upper)*range[dim];
    }

    T fun = gpu::apply(*d_integrand, x)* (*jacobian);
    sdata[pIndex] = fun; // target for reduction //this assumes each thread has
  a spot in sdata, but threads enter if they are larger than FEVAL, this would
  mean threads that arent' supposed to enter, enter this computation adn we
  ...
  ___SampleRegionBlock(IntegT* d_integrand,
                    ...
                    int FEVAL,
                    int NSETS,
                    Region<NDIM> sRegionPool[], double* vol, int* maxdim, double
  range[],  double* jacobian, double* generators, int iteration)
  {
    constexpr int feval = (1 + 2 * NDIM + 2 * NDIM + 2 * NDIM + 2 * NDIM +
                        2 * NDIM * (NDIM - 1) + 4 * NDIM * (NDIM - 1) +
                        4 * NDIM * (NDIM - 1) * (NDIM - 2) / 3 + (1 << NDIM));
    __shared__ double sdata[feval]; //unused memory at the end feval-4*NDIM-1 is
  unused, done this way to avoid branching when assigning Region<NDIM>* const
  region = (Region<NDIM>*)&sRegionPool[sIndex]; T g[NDIM];
    gpu::cudaArray<double, NDIM> x;
    int perm = 0;

    T ratio = Sq(__ldg(&constMem._gpuG[2 * NDIM]) / __ldg(&constMem._gpuG[1 *
  NDIM])); int offset = 2 * NDIM;
    ...
    if (pIndex < FEVAL) {
      computePermutation<IntegT, T, NDIM>(
        d_integrand, pIndex, perm, region->bounds, g, x, sum, constMem, range,
  jacobian, generators, FEVAL, iteration, sdata);
    }
  ...
  jacobian, generators, FEVAL, iteration, sdata);
    ...
    __syncthreads(); //can we get rid of these, each thread writes to
  sdata[threadIdx.x], shouldn't be race condition
    ...
    if (threadIdx.x == 0) {
      ...
      for (int rul = 1; rul < NRULES - 1; ++rul) {
        ...
        for (int s = 0; s < NSETS; ++s) {
          maxerr = max(maxerr,
                       fabs(sum[rul + 1] +
                            __ldg(&constMem._GPUScale[s * NRULES + rul]) *
  sum[rul]) *
